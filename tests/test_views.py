...
class TestApiViews(SupersetTestCase):
    ...
    def setUp(self):
        ...
        superset.security_manager.add_user('test_user', 'Test', 'User',
                                           'test@user.org', test_role,
                                           password='password')
        superset.security_manager.get_session.commit()
        test_role = superset.security_manager.find_role('test_role')
        helpers.grant_api_permission_to_role(test_role, SupersetKetchupApiView,
                                             'all_dashboards')
    ...
    def login(self, username, password='password'):
        ...
        login_response = self.client.post(
            "/login/", data=dict(username=username,
    ...
    def test_all_dashboards_owned(self):  # pylint: disable=R0201
        ...
        response = _jsonify(
            self.client.get("/superset-ketchup/api/all_dashboards/"))
    ...
    def test_all_dashboards_no_access(self):  # pylint: disable=R0201
        ...
        response = _jsonify(
            self.client.get("/superset-ketchup/api/all_dashboards/"))
        assert response == {'message': 'Access is Denied',
                            'severity': 'danger'}
